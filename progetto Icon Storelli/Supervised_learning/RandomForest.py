...
X = data_dummies
# Creiamo un target binario 0/1 in modo robusto, filtrando eventuali record invalidi
y = dataset['Class/ASD'].map({'1': 1, '0': 0})
# Se ci fossero ancora valori non mappabili, rimuovili e riallinea X e y
mask = y.notna()
if not mask.all():
    dropped = (~mask).sum()
