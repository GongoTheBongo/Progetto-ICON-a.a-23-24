...
KNN_N_JOBS = int(os.environ.get("KNN_N_JOBS", "-1"))
...
grid_search = _make_grid_search(KNN_N_JOBS)
...
plt.tight_layout()
plt.savefig(_with_dataset_suffix('knn_confusion_matrix.png'), dpi=300)
...
best_cv_scores = [
    results_df.loc[best_idx, f'split{i}_test_score']
...
plt.tight_layout()
plt.savefig(_with_dataset_suffix('knn_roc_curve.png'), dpi=300)
...
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(_with_dataset_suffix('knn_precision_recall_curve.png'), dpi=300)
...
EXPERIMENT_CSV = os.environ.get(
    "KNN_EXPERIMENT_CSV",
    _with_dataset_suffix("knn_experiment_results.csv")
)
...
experiment_meta = {
    ...
    "test_accuracy": float(accuracy_grid),
    "test_precision": float(precision_grid),
    "test_recall": float(recall_grid),
    "test_f1": float(f1_grid),
    "test_auc": float(auc),
...
print(f"\nRisultati esperimento KNN salvati in: {EXPERIMENT_CSV}")
...
summary_row = {"model": "KNN", **experiment_meta}
