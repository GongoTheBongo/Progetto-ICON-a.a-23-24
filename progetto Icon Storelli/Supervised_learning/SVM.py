...
# Numero di job per GridSearchCV (puoi forzarlo con env var SVM_N_JOBS)
SVM_N_JOBS = int(os.environ.get("SVM_N_JOBS", "-1"))
...
grid_search = _make_grid_search(SVM_N_JOBS)
...
plt.tight_layout()
plt.savefig(_with_dataset_suffix('svm_confusion_matrix.png'), dpi=300)
...
plt.tight_layout()
plt.savefig(_with_dataset_suffix('svm_roc_curve.png'), dpi=300)
...
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(_with_dataset_suffix('svm_precision_recall_curve.png'), dpi=300)
...
EXPERIMENT_CSV = os.environ.get(
    "SVM_EXPERIMENT_CSV",
    _with_dataset_suffix("svm_experiment_results.csv")
)
...
print(f"\nRisultati esperimento SVM salvati in: {EXPERIMENT_CSV}")
...
summary_row = {"model": "SVM", **experiment_meta}
